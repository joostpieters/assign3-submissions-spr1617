...
def get_recipe(recipe_endpoint):
    ...
    if response.ok == False:
        raise Exception("Failed to get a response from allrecipes.com!")
    soup = BeautifulSoup(response.content, 'html.parser')
    ...
    ingredients = [tag.string.strip() for tag in soup.find_all(attrs={'class' :'recipe-ingred_txt'}) if tag.attrs.get('itemprop') != None and tag.attrs['itemprop'] == 'ingredients']
    ...
    for tag in soup.find_all('time'):
        ...
        if totalTime == None:
            ...
            continue
    instructions = [tag.string.strip() for tag in soup.find_all(attrs={'class': 'recipe-directions__list--item'}) if tag.string and len(tag.string.strip()) > 0]
