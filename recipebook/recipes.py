...
try:
    ...
    print('First, tell us whether we should use a local cache of recipes,'
          ' or if we should instead go to allrecipes.com '
          'to answer your queries.')
    print('Then, ask us to search for a given recipe.'
          ' We know how to search by keyword, '
          'and by including and excluding ingredients.')
    print('The format for such a query'
          ' is "keyword1 keyword2 +ingredientToInclude1 '
          '+ingredientToInclude2 -ingredientToExclude1 '
          '-ingredientToExclude2')
    ...
    user_input = input(
        ...
        'instead of a local copy of recipes? Enter Y/N: ').strip().upper()
    while not user_input == 'Y' and not user_input == 'N':
        print('That wasn\'t either a Y or an N! Try again.')
        user_input = input(
            'Should we use allrecipes.com '
            'for recipes? Enter Y/N: ').strip().upper()
    if user_input == 'Y':
        ...
        if search:
            if should_use_all_recipes():
                ...
                recipes = fetcher.fetch_recipes(
                    *keywords, includeIngredients=includes,
                ...
                print("Found {size} matching recipes.".format(
                    size=last_book.size()))

        else:  # Organize
            if last_book is None or last_book.size() == 0:
                print(
                    'Before you can organize some search results,'
