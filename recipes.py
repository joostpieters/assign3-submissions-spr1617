...
def invalid_yn_input(str):
    return not str or str[0] not in "YN"

def search_recipes_input():
    action = input('(S)earch/(O)rganize? ')
    while action[0].upper() not in "SO":
        action = input('(S)earch/(O)rganize? ')
...
try:
    ...
    while True:
        ...
        while invalid_yn_input(do_continue):
